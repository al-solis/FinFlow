...
class AccountingService
{
            ...
            foreach ($details->groupBy('vendor_id') as $vendorId => $vendorLines) {
                ...
                foreach ($vendorLines as $detail) {
                    gl_journal_line::create([
                        ...
                        'subledger_type' => \App\Models\rfd_detail::class,
                ...
                foreach ($invoiceLinesData as $lineData) {
                    ap_invoice_line::create([
                        ...
                        'source_line_type' => \App\Models\rfd_detail::class,
     ...
     * Creates a balanced GL journal (Dr AP / Cr Bank).
     */
    public function postDisbursement(
        ...
        return DB::transaction(function () use ($apInvoiceIds, $bankAccountId, $paymentMethodId, $referenceNumber, $checkDate, $actorId) {
            $invoices = ap_invoice::whereIn('id', $apInvoiceIds)->lockForUpdate()->get();
            ...
            $this->assertBalanced($totalAmount, $totalAmount);
            ...
            foreach ($invoices as $invoice) {
                ...
                $invoice->applyPayment($invoice->amount_due);
    ...
    public function postBankStatementLine(
        ...
        return DB::transaction(function () use ($bankAccount, $line, $offsetGlAccountId, $actorId) {
            ...
            // Deposit: Debit Bank, Credit offsetting account (income/AR/etc.)
            // Withdrawal/charge: Debit offsetting account (expense/AP/etc.), Credit Bank
            gl_journal_line::create([
                'gl_journal_id' => $journal->id,
                'line_no' => 1,
