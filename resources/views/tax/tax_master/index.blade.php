...
@section('content')
    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Tax Masters</h1>
                <p class="text-sm text-gray-500">
                    Maintain tax definitions.
                </p>
            </div>
                    ...
                    <select id="searchtype" name="searchtype"
                        ...
                        @foreach ($taxTypes as $taxType)
                            <option value="{{ $taxType->id }}"
                                {{ request('searchtype') == $taxType->id ? 'selected' : '' }}>{{ $taxType->name }}</option>
                        @endforeach
                    </select>
                </div>

                <div class="md:w-1/3 w-full">
                    <select id="searchformula" name="searchformula"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                        onchange="this.form.submit()">
                        <option value="">All Tax Formulas</option>
                        @foreach ($taxFormulas as $taxFormula)
                            <option value="{{ $taxFormula->id }}"
                                {{ request('searchformula') == $taxFormula->id ? 'selected' : '' }}>{{ $taxFormula->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <table class="min-w-full text-xs">
                <thead class="bg-gray-200 text-gray-600">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-left w-[80px]">Formula</th>
                        <th scope="col" class="px-4 py-3 text-left w-[80px]">Rate</th>
                        <th scope="col" class="px-4 py-3 text-left w-[80px]">Fixed Amount</th>
                        <th scope="col" class="px-4 py-3 text-left w-[120px]">GL Account Code</th>
                ...
                <tbody class="divide-y">
                    @forelse($taxMasters as $taxMaster)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 w-[80px]">{{ $taxMaster->code }}</td>
                            <td class="px-4 py-3 w-[150px]">{{ $taxMaster->name }}</td>
                            <td class="px-4 py-3 w-[80px]">{{ $taxMaster->taxType->code }}</td>
                            <td class="px-4 py-3 w-[80px]">{{ $taxMaster->taxFormula->code }}</td>
                            <td class="px-4 py-3 text-right w-[80px]">
                                {{ $taxMaster->rate != 0 ? number_format($taxMaster->rate, 2) : '' }}</td>
                            <td class="px-4 py-3 text-right w-[80px]">
                            ...
                            <td class="px-4 py-3 w-[100px] text-xs font-semibold">
                                @php
                                    ...
                                    $status = $statuses[$taxMaster->status] ?? [
                            ...
                            <td class="px-4 py-3 w-[50px]">
                                <div class="flex items-center justify-center space-x-2">
                                    <button type="button" title="Edit tax master {{ $taxMaster->name }}"
                                        data-modal-target="edit-modal" data-modal-toggle="edit-modal"
                                        data-id="{{ $taxMaster->id }}" data-name="{{ $taxMaster->name }}"
                                        data-code="{{ $taxMaster->code }}" data-tax_type="{{ $taxMaster->taxType->id }}"
                                        ...
                                        data-effective_to="{{ $taxMaster->effective_to }}"
                                        data-status="{{ $taxMaster->status }}" onclick="openEditModal(this)"
        ...
        <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
            ...
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <div class="overflow-y-auto max-h-[70vh]">
                    <form action="{{ route('tax.tm.store') }}" method="POST">
                        @csrf
                        <div class="grid ml-1 mr-1 gap-2 mb-4 sm:grid-cols-2">
                            ...
                            <div class="md:col-span-1">
                                <label for="tax_type_id"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Tax Type*</label>
                                <select id="tax_type_id" name="tax_type_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    required>
                                    <option value="">Select tax type</option>
                                    @foreach ($taxTypes as $taxType)
                                        <option value="{{ $taxType->id }}">{{ $taxType->code }} - {{ $taxType->name }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="md:col-span-1">
                                <label for="tax_formula_id"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Tax Formula*</label>
                                <select id="tax_formula_id" name="tax_formula_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    required>
                                    <option value="">Select tax formula</option>
                                    @foreach ($taxFormulas as $taxFormula)
                                        <option value="{{ $taxFormula->id }}">{{ $taxFormula->code }} -
                                            {{ $taxFormula->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="sm:col-span-1">
                                <label for="rate" class="block text-xs font-medium text-gray-900 dark:text-white">Rate
                                    (%)*</label>
                                <input type="number" name="rate" id="rate" min="0" step="0.01"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                            ...
                            <div class="sm:col-span-1">
                                <label for="fixed_amount"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Fixed Amount
                                </label>
                                <input type="number" name="fixed_amount" id="fixed_amount" min="0"
                                    step="0.01"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    placeholder="e.g. 100">
                            </div>
                            ...
                            <div class="sm:col-span-1">
                                <label for="effective_from"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Effective From*</label>
                            ...
                            <div class="sm:col-span-1">
                                <label for="effective_to"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Effective To</label>
                            ...
                            <div class="sm:col-span-1">
                                <label for="priority"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Priority
                                </label>
                                <input type="number" name="priority" id="priority" min="1" step="1"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                            ...
                            <div class="flex items-center mb-4">
                                <input id="recoverable" name="recoverable" type="checkbox" value=""
                                    class="w-4 h-4 border border-default-medium rounded-xs bg-neutral-secondary-medium focus:ring-2 focus:ring-brand-soft">
                                <label for="recoverable"
                                    class="block text-xs font-medium text-gray-900 dark:text-white ml-2">Recoverable</label>
                            </div>
                ...
                <div class="overflow-y-auto max-h-[70vh]">
                    <form id="editForm" class="p-4 md:p-5" method="POST">
                        ...
                        <div class="grid ml-1 mr-1 gap-2 mb-4 sm:grid-cols-2">
                            ...
                            <div class="md:col-span-1">
                                <label for="edit_tax_type_id"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Tax Type*</label>
                                <select id="edit_tax_type_id" name="edit_tax_type_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    required>
                                    <option value="">Select tax type</option>
                                    @foreach ($taxTypes as $taxType)
                            ...
                            <div class="md:col-span-1">
                                <label for="edit_tax_formula_id"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Tax Formula*</label>
                                <select id="edit_tax_formula_id" name="edit_tax_formula_id"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-500 focus:border-gray-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    required>
                                    <option value="">Select tax formula</option>
                                    @foreach ($taxFormulas as $taxFormula)
                            ...
                            <div class="sm:col-span-1">
                                <label for="edit_rate"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Rate
                                    (%)*</label>
                                <input type="number" name="edit_rate" id="edit_rate" min="0" step="0.01"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    placeholder="e.g. 18%" required>
                            </div>

                            <div class="sm:col-span-1">
                                <label for="edit_fixed_amount"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Fixed Amount
                                </label>
                                <input type="number" name="edit_fixed_amount" id="edit_fixed_amount" min="0"
                                    step="0.01"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    placeholder="e.g. 100">
                            </div>
                            ...
                            <div class="sm:col-span-1">
                                <label for="edit_effective_from"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Effective From*</label>
                                <input type="date" name="edit_effective_from" id="edit_effective_from"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    required>
                            </div>

                            <div class="sm:col-span-1">
                                <label for="edit_effective_to"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Effective To</label>
                                <input type="date" name="edit_effective_to" id="edit_effective_to"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500">
                            </div>
                            ...
                            <div class="sm:col-span-1">
                                ...
                                <input type="number" name="edit_priority" id="edit_priority" min="1"
                                    step="1"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    placeholder="e.g. 1" required>
                            </div>
                            ...
                            <div class="flex items-center mb-4">
                                <input id="edit_recoverable" name="edit_recoverable" type="checkbox" value=""
                                    class="w-4 h-4 border border-default-medium rounded-xs bg-neutral-secondary-medium focus:ring-2 focus:ring-brand-soft">
                                <label for="edit_recoverable"
                                    class="block text-xs font-medium text-gray-900 dark:text-white ml-2">Recoverable</label>
                            </div>
                        </div>
    ...
    <script>
        ...
        function openEditModal(button) {
            ...
            document.getElementById('edit_tax_type_id').value = button.getAttribute('data-tax_type');
            document.getElementById('edit_tax_formula_id').value = button.getAttribute('data-tax_formula');
