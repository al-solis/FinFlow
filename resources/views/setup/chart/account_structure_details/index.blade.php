    ...
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500">{{ $accountStructure->name }}</p>
            </div>
            ...
            <table class="min-w-full text-xs">
                <thead class="bg-gray-200 text-gray-600">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left">Segment Name</th>
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    @if (!$loop->first)
                                        <form method="POST"
                                            action="{{ route('setup.chart.account_structure_details.move-up', [$accountStructure->id, $detail->id]) }}">
                                            @csrf @method('PUT')
                                    ...
                                    @if (!$loop->last)
                                        <form method="POST"
                                            action="{{ route('setup.chart.account_structure_details.move-down', [$accountStructure->id, $detail->id]) }}">
                                            @csrf @method('PUT')
                                    ...
                                    @if ($detail->source_type != 'main_account')
                                        <form method="POST"
                                            action="{{ route('setup.chart.account_structure_details.destroy', [$accountStructure->id, $detail->id]) }}"
        ...
        <div class="bg-white border rounded-xl p-5 space-y-4">
            <h3 class="font-semibold text-gray-900">Account Code Format Preview</h3>
    ...
    <div id="add-detail-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md h-full md:h-auto">
            <div class="relative p-4 bg-white rounded-lg shadow sm:p-5">
                ...
                <form action="{{ route('setup.chart.account_structure_details.store', $accountStructure->id) }}"
                    method="POST">
                    @csrf
                    <div class="mb-4">
