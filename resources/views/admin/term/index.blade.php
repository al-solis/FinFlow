...
@section('content')
    <div class="p-6 space-y-6">
        ...
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900">Payment Terms</h1>
            ...
            <table class="min-w-full text-xs">
                <thead class="bg-gray-200 text-gray-600">
                    <tr>
                        ...
                        <th scope="col" class="px-4 py-3 text-left w-[100px]">Days</th>
                ...
                <tbody class="divide-y">
                    @forelse($terms as $term)
                        <tr class="hover:bg-gray-50">
                            <td class="px-4 py-3 w-[80px]">{{ $term->code }}</td>
                            <td class="px-4 py-3 w-[150px]">{{ $term->name }}</td>
                            <td class="px-4 py-3 w-[200px]">{{ $term->description }}</td>
                            <td class="px-4 py-3 w-[100px]">{{ $term->days }}</td>
                            ...
                            <td class="px-4 py-3 w-[50px]">
                                <div class="flex items-center justify-center space-x-2">
                                    <button type="button" title="Edit terms {{ $term->name }}"
                                        data-modal-target="edit-modal" data-modal-toggle="edit-modal"
                                        data-id="{{ $term->id }}" data-name="{{ $term->name }}"
                                        data-code="{{ $term->code }}" data-description="{{ $term->description }}"
                                        data-days="{{ $term->days }}" data-status="{{ $term->status }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-gray-500">
                                <img src="{{ asset('images/terms.svg') }}" alt="No data" class="mx-auto mb-4 w-24 h-28">
                                No payment terms found. Click here to <a href="#" data-modal-target="add-modal"
                    ...
                    <form action="{{ route('admin.terms.store') }}" method="POST">
                        ...
                        <div class="grid ml-1 mr-1 gap-2 mb-4 sm:grid-cols-2">
                            ...
                            <div class="md:col-span-1">
                                <label for="days"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Days*</label>
                    ...
                    <form id="editForm" class="p-4 md:p-5" method="POST">
                        ...
                        <div class="grid ml-1 mr-1 gap-2 mb-4 sm:grid-cols-2">
                            ...
                            <div class="md:col-span-1">
                                <label for="edit_days"
                                    class="block text-xs font-medium text-gray-900 dark:text-white">Days*</label>
                                <input type="number" name="edit_days" id="edit_days" min="0"
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-lg focus:ring-gray-600 focus:border-gray-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-gray-500 dark:focus:border-gray-500"
                                    placeholder="e.g. 1" required>
                            </div>
    ...
    <script>
        ...
        function openEditModal(button) {
            ...
            document.getElementById('edit_days').value = button.getAttribute('data-days');
