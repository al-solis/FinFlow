    ...
    <x-slot name="header">
        <div class="py-0.5">
            <div class="flex flex-wrap justify-center gap-1.5 sm:gap-2">
                <!-- Dashboard Link -->
                <a href="{{ route('mainDashboard') }}" title="Real-time overview of agency assets and property"
                    ...
                    <img width="24" height="24" src="{{ asset('icons/dashboard.png') }}" alt="Dashboard" />
                ...
                @if ($authService->canRead(Modules::APPW, Modules::APPW_APPR))
                    <a href="{{ route('approvals.index') }}"
                        ...
                        class="py-1.5 px-2.5 flex flex-col items-center gap-x-1.5 text-sm text-gray-800 bg-gray-100 hover:text-cyan-700 rounded-lg focus:outline-hidden focus:text-cyan-700 hover:bg-gray-200 relative">
                        <div class="flex flex-col items-center relative">
                            <img width="24" height="24" src="{{ asset('icons/approvals.png') }}"
                                alt="Approvals" />
                            @if (isset($globalPendingApprovals) && $globalPendingApprovals > 0)
                                <span
                                    class="absolute -bottom-3 -right-2 bg-green-500 text-white text-xs font-bold rounded-full min-w-[18px] h-[18px] px-1 flex items-center justify-center shadow-md border-2 border-white z-10">
                    ...
                    @if ($hasAccessibleSubModules)
                        <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                            <button type="button" @click="open = !open" title="{{ $module->description }}"
                                class="py-1.5 px-2.5 flex flex-col items-center gap-x-1.5 text-sm text-gray-800 bg-gray-100 hover:text-cyan-700 rounded-lg focus:outline-hidden focus:text-cyan-700 hover:bg-gray-200">
                                <img width="24" height="24" src="{{ asset('icons/' . $module->img) }}"
                                    alt="{{ $module->name }}" />
                ...
                <a href="{{ route('reports.index') }}" title="Financial Reports"
                    ...
                    <img width="24" height="24" src="{{ asset('icons/reports.png') }}" alt="Reports" />
